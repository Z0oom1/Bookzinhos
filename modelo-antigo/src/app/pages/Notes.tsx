...
const quickFeedback = ["Amei 🐼", "Confuso 🐼", "Viciante 🐼", "Emocionante 🐼", "Chocante 🐼"];
      ...
      <div className="flex gap-2 justify-center">
        {Array.from({ length: 5 }).map((_, i) => (
          <button
            ...
            className={`text-4xl transition-transform hover:scale-110 active:scale-95 ${i < displayRating ? "" : "opacity-30 grayscale"}`}
          >
            🐼
          </button>
        ))}
            ...
            <div className="space-y-5">
              ...
              {notes.length === 0 ? (
                <div className="text-center py-12 text-[var(--text-muted)] bg-white/40 backdrop-blur-sm rounded-[2rem] border-2 border-dashed border-[var(--lavender)]/20 shadow-sm font-medium">
                  Nenhuma notinha ainda. Que tal escrever a primeira? 🐼
                </div>
              ) : (
                notes.map((note) => (
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex bg-white/50 px-2 py-1 rounded-full shadow-sm">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <span key={i} className={`text-sm ${i < note.rating ? "" : "opacity-30 grayscale"}`}>🐼</span>
                          ))}
                        </div>
                      </div>
                      <button
